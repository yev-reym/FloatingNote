...
const leftPlayer = (props) => {
    ...
    const scrubberStatus = () => {
        if (props.playing) {
            props.Scrubber.current.pause();
            props.pause();
        } else {
            props.Scrubber.current.play();
            props.play();
        }
    };

    const status = props.playing ? pauseSrc : playSrc;

    return (
        <section className="controls-container">
            <img src={prevSrc} className='prev-button' alt="prev" />
            <img src={status} className='track-play-button' alt="play" onClick={scrubberStatus}/>
